...
#读取月平均数据
print('Read data1')
ta_ts_year = pd.read_excel('ta_ts_month.xlsx', header=0, names=['STA_NAME', 'STATION',
                           'LONGTITUDE', 'LATITUDE', 'ALTITUDE', 'year', 'month',
                         'ta', 'meantas','LT5cm', 'LT10cm', 'LT15cm', 'LT20cm', 'LT40cm',
                         'LT80cm', 'LT160cm', 'LT320cm', 'maxtas', 'mintas', 'prec', 'wind'])
...
for var,lag in zip(varlist,laglist):
    ...
    print('lagging series')
    data_input_ = np.empty(shape=data_input.shape)
    data_input_[:,-2:] = data_input[:,-2:]
    for i in range(len(data_input)):
        if i-lag < 0 :#最前面的时次
            data_input_[i,:-2] = np.nan
        elif meantas['STATION'][i-lag] != meantas['STATION'][i]:
            data_input_[i,:-2] = np.nan
        else:
            data_input_[i,:-2] = data_input[i - lag,:-2]
    print(data_input_.shape)
